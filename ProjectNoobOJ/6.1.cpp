...
public:
    TA(string nu, string na, string s, string b, string i, int c, string sub, string p, string d) : Student(nu, na, s, b, i, c), Teacher(nu,na,s,b,i,p, d) {}
    //编号，姓名，性别，出生年月，身份证号，班号，专业，职务，部门
    TA(const TA& T): Student(T), Teacher(T) {}
    ~TA() {}
    void Show()const {
        cout << "编号:" << number << endl
            << "姓名:" << name << endl
